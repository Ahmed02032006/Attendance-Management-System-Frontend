...
import { getSchoolsByUserId } from '../../store/Admin-Slicer/Admin-School-Slicer';
...
const OnboardingPage = () => {
  ...
  if(schoolId){
    return <Navigate to={`/admin/dashboard/${schoolId}`} />
  }
                ...
                {schools && schools.length ? (
                  schools.map((school) => (
                    <div
                      ...
                      key={school._id}
                      className="relative flex items-center bg-white rounded-lg border border-gray-200 shadow-sm p-4 transition-all mb-3 hover:shadow-md cursor-pointer"
                    >
                      {/* Verification Status Badge - Top Right Inside Box */}
                      <div className={`absolute top-0 right-0 px-2 py-1 rounded-tr-sm rounded-bl-sm text-xs font-medium flex items-center ${school.verificationStatus === 'Verified'
                      ...
                      {/* External Link Button (if website exists) */}
                      {school.website && (
                        <button
                          onClick={(e) => handleWebsiteClick(e, school.website)}
                          className="ml-2 mt-5 text-gray-400 hover:text-sky-500 p-1 rounded-full hover:bg-gray-100 transition-colors"
