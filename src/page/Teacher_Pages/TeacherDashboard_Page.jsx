...
const TeacherDashboard_Page = () => {
  ...
  const callAIApi = async (userQuery) => {
    try {
      ...
      // Format the response with clickable page names
      let aiResponseText = data.response;

      // Replace full URLs with clickable page names
      aiResponseText = aiResponseText
        .replace(/https:\/\/attendance-management-system-fronte-two\.vercel\.app\/teacher\/dashboard/g, 
                 '<a href="/teacher/dashboard" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Dashboard</a>')
        .replace(/https:\/\/attendance-management-system-fronte-two\.vercel\.app\/teacher\/subject/g, 
                 '<a href="/teacher/subject" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Subjects</a>')
        .replace(/https:\/\/attendance-management-system-fronte-two\.vercel\.app\/teacher\/attendance/g, 
                 '<a href="/teacher/attendance" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Attendance</a>')
        .replace(/\/teacher\/dashboard/g, 
                 '<a href="/teacher/dashboard" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Dashboard</a>')
        .replace(/\/teacher\/subject/g, 
                 '<a href="/teacher/subject" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Subjects</a>')
        .replace(/\/teacher\/attendance/g, 
                 '<a href="/teacher/attendance" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Attendance</a>')
        .replace(/dashboard page/g, 
                 '<a href="/teacher/dashboard" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Dashboard</a>')
        .replace(/subject page/g, 
                 '<a href="/teacher/subject" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Subjects</a>')
        .replace(/attendance page/g, 
                 '<a href="/teacher/attendance" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Attendance</a>');
      ...
      // Smart fallback responses with clickable links
      const lowerQuery = userQuery.toLowerCase();

      if (lowerQuery.includes('attendance') && lowerQuery.includes('add')) {
        return `To add attendance:\n\n1. Navigate to the <a href="/teacher/dashboard" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Dashboard</a> from the main menu\n2. Go to the <a href="/teacher/subject" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Subjects</a> page\n3. Select a subject\n4. Click on the <a href="/teacher/attendance" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Attendance</a> page\n5. Choose the date for attendance\n6. Mark students as present/absent\n7. Click "Save Attendance"\n\nYou currently have ${dashboardSubjects.length} subjects assigned.`;

      } else if (lowerQuery.includes('view') && lowerQuery.includes('record')) {
        return `To view attendance records:\n\n1. Select a subject from the left panel\n2. Use date navigation to select a specific date\n3. View student records in the table\n4. Use pagination if there are many students\n\nCurrently showing ${currentAttendanceRecords.length} records for ${selectedSubjectData?.name || 'selected subject'}.`;

      } else if (lowerQuery.includes('dashboard') || lowerQuery.includes('overview')) {
        return `Your <a href="/teacher/dashboard" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Dashboard</a> shows:\n\n• ${dashboardSubjects.length} assigned subjects on the left\n• Attendance records for selected subject on the right\n• Date navigation for historical records\n• Student details with roll numbers and timestamps\n\nSelect a subject to see detailed attendance.`;

      } else if (lowerQuery.includes('subject') || lowerQuery.includes('course')) {
        return `You have ${dashboardSubjects.length} subjects:\n${dashboardSubjects.map((sub, i) => `${i + 1}. ${sub.name} (${sub.code}) - ${sub.students || 0} students`).join('\n')}\n\nClick on any subject to view its attendance records. You can also visit the <a href="/teacher/subject" target="_blank" class="text-sky-600 hover:text-sky-800 underline font-medium">Subjects</a> page for more details.`;
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const fallbackResponse = {
        id: Date.now() + 1,
        text: "I'm having trouble connecting right now. Here are some common actions:\n\n1. To add attendance: Go to the <a href='/teacher/attendance' target='_blank' class='text-sky-600 hover:text-sky-800 underline font-medium'>Attendance</a> page\n2. To view records: Select a subject above\n3. To see student details: Check the table below\n\nPlease try your query again or contact support if the issue persists.",
        sender: 'assistant',
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      }
                    ...
                    <div 
                      ...
                      dangerouslySetInnerHTML={{ 
                        __html: msg.sender === 'assistant' 
                          ? msg.text.replace(/\n/g, '<br>') 
                          : msg.text.replace(/\n/g, '<br>').replace(/<a /g, '<a style="color: inherit; text-decoration: underline;" ') 
                      }} 
                    />
                    <p className="text-xs mt-1 opacity-70">
                      {msg.sender === 'user' ? 'You' : 'Assistant'} • {msg.timestamp}
