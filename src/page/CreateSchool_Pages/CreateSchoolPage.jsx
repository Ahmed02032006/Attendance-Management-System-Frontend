...
import { createNewSchool } from '../../store/Admin-Slicer/Admin-School-Slicer';
...
const CreateSchoolPage = () => {
  ...
  const onSubmit = (e) => {
    ...
    dispatch(createNewSchool(myNewFormData))
      .then((res) => {
        if (res.payload?.status === "Success") {
          toast.success('Your New School Created Successfully');
          setTimeout(() => {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-gradient-to-br from-sky-50 to-white overflow-auto">
      ...
      <div className="hidden md:block md:w-full lg:w-[55%] relative overflow-hidden h-[300px] md:h-full">
        ...
        {/* Enhanced slider indicators with progress animation */}
        <div className="absolute bottom-6 lg:bottom-10 left-0 right-0 flex justify-center space-x-2 z-20">
          {images.map((_, index) => (
            <button
              key={index}
              onClick={() => setCurrentSlide(index)}
              className="relative h-1 rounded-full overflow-hidden bg-white/30 w-12"
        ...
        <div className="absolute top-6 left-6 z-20 opacity-80">
          <div className="flex items-center">
            <svg className="w-6 h-6 text-sky-300 mr-2" fill="currentColor" viewBox="0 0 20 20">
              <path d="M10 2a8 8 0 100 16 8 8 0 000-16zm0 14a6 6 0 110-12 6 6 0 010 12z" />
              <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
            </svg>
            <span className="text-white font-medium">Manage Together</span>
          </div>
        </div>
      </div>
      ...
      <div className="w-full lg:w-[45%] flex items-center justify-center overflow-y-auto bg-white py-8 md:py-0">
        <div className="w-full max-w-md px-4 py-6">
          ...
          focus:outline-none"
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <label htmlFor="schoolName" className="block text-xs sm:text-sm font-medium text-gray-700">
                  School Name
                </label>
              </div>
              <input
